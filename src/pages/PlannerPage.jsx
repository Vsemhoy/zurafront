...
function UnscheduledTray({ tasks, onDragStart, onDragEnd, onOpen, onClose }) {
    const [search, setSearch] = useState('');
    const needle = search.trim().toLowerCase();
    const filtered = needle ? tasks.filter((task) => `${task.task_key} ${task.title} ${task.project?.title ?? ''}`.toLowerCase().includes(needle)) : tasks;
    return <aside className="planner-unscheduled"><header><div><IconListDetails size={16}/><strong>Незапланированные</strong></div><span><small>{filtered.length}{filtered.length !== tasks.length ? ` из ${tasks.length}` : ''}</small><button type="button" title="Скрыть незапланированные" aria-label="Скрыть незапланированные" onClick={onClose}><IconX size={15}/></button></span></header><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Найти задачу…"/><p>Перетащите задачу на нужный день.</p><div>{filtered.map((task) => { const status = taskStatusMap[task.status] ?? taskStatusMap.todo; return <article key={task.id} draggable style={{ background: status.color, borderLeftColor: task.project?.color ?? '#8794a7' }} title="Перетащить в календарь · двойной клик — полный редактор" onDragStart={(event) => { onDragStart({ kind: 'task', id: task.id, task }); event.dataTransfer.effectAllowed = 'copyMove'; event.dataTransfer.setData('text/plain', task.id); }} onDragEnd={onDragEnd} onDoubleClick={() => onOpen(task)}><span><b>{taskReference(task)}</b>{task.project?.key && <i>{task.project.key}</i>}</span><strong>{task.title}</strong>{task.assignee?.name && <small>{task.assignee.name}</small>}</article>; })}{!filtered.length && <div className="planner-unscheduled__empty">{tasks.length ? 'Ничего не найдено' : 'Все рабочие задачи уже распределены'}</div>}</div></aside>;
}
...
export function PlannerPage() {
    ...
    useEffect(() => {
        localStorage.setItem('zuratax:planner-unscheduled', queueOpen ? 'open' : 'closed');
    }, [queueOpen]);
    useEffect(() => {
    ...
    const selectTaskGroup = (taskId, additive = false) => setSelected((current) => {
        if (!additive) return [taskId];
