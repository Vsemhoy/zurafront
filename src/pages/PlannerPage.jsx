...
function MultiFilter({ label, items, selected, onChange }) {
    return <details className="planner-filter"><summary><IconFilter size={14}/><span>{selected.length ? `${label}: ${selected.length}` : label}</span><IconChevronDown size={14}/></summary><div className="planner-filter__menu"><button type="button" onClick={() => onChange([])}>Сбросить</button>{items.map((item) => <label key={item.value}><input type="checkbox" checked={selected.includes(item.value)} onChange={() => onChange(selected.includes(item.value) ? selected.filter((value) => value !== item.value) : [...selected, item.value])}/><i style={{ background: item.color }}/><span>{item.label}</span></label>)}</div></details>;
}

function QuickCreate({ date, projects, assignees, defaults, onClose, onCreate, pending, error }) {
    const [form, setForm] = useState({ title: '', project_id: defaults.projectId ?? '', assignee_id: defaults.assigneeId ?? '', status: defaults.status ?? 'scheduled', priority: 2 });
    return <div className="planner-modal-backdrop" onMouseDown={onClose}><form className="planner-modal" onMouseDown={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); onCreate({ ...form, project_id: form.project_id || null, assignee_id: form.assignee_id || null, priority: Number(form.priority), due_at: `${date} 12:00:00` }); }}><header><div><small>Быстрое планирование</small><h2>{Number(date.slice(8))} {monthNames[Number(date.slice(5, 7)) - 1]}</h2></div><button type="button" onClick={onClose}><IconX size={18}/></button></header><label>Задача<input autoFocus required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Что должно быть сделано?"/></label><div className="planner-modal__grid"><label>Проект<select value={form.project_id} onChange={(event) => setForm({ ...form, project_id: event.target.value })}><option value="">Без проекта</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.title}</option>)}</select></label><label>Исполнитель<select value={form.assignee_id} onChange={(event) => setForm({ ...form, assignee_id: event.target.value })}><option value="">Не назначен</option>{assignees.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select></label><label>Статус<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })}>{taskStatuses.filter((status) => status.value !== 'blocked').map((status) => <option key={status.value} value={status.value}>{status.label}</option>)}</select></label><label>Приоритет<select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })}>{[1, 2, 3, 4, 5].map((priority) => <option key={priority} value={priority}>{priorityLabel(priority)}</option>)}</select></label></div>{error && <p className="planner-error">{error.message}</p>}<footer><button type="button" onClick={onClose}>Отмена</button><button className="planner-primary" disabled={pending}><IconPlus size={16}/>{pending ? 'Создаю…' : 'Создать'}</button></footer></form></div>;
}
...
export function PlannerPage() {
    ...
    const createTask = useMutation({ mutationFn: (payload) => taskApi.create(scopeId, payload), onSuccess: () => { setCreateDate(null); refresh(); } });
    ...
    return <main className={`planner-page ${bulkOpen ? 'planner-page--bulk' : ''}`}>
        ...
        {createDate && <QuickCreate date={createDate} projects={projects} assignees={assignable.assignees} defaults={{ projectId: projectIds[0], assigneeId: assigneeIds[0], status: statuses[0] }} onClose={() => setCreateDate(null)} onCreate={(payload) => createTask.mutate(payload)} pending={createTask.isPending} error={createTask.error}/>} 
    </main>;
}
